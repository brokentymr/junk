// Strategy Icons as SVG Components - Cute & Playful

export const MomentumIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M3 12L7 8L12 13L17 8L21 12" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M3 18L7 14L12 19L17 14L21 18" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    <circle cx="7" cy="8" r="2" fill="currentColor" opacity="0.6"/>
    <circle cx="17" cy="8" r="2" fill="currentColor" opacity="0.6"/>
  </svg>
)

export const ArbitrageIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2.5"/>
    <path d="M8 12L11 15L16 9" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    <circle cx="7" cy="7" r="2.5" fill="currentColor" opacity="0.8"/>
    <circle cx="17" cy="17" r="2.5" fill="currentColor" opacity="0.8"/>
    <circle cx="12" cy="12" r="1.5" fill="currentColor"/>
  </svg>
)

export const MeanReversionIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M3 12H21" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
    <path d="M12 3V21" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
    <path d="M6 9L12 3L18 9" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M6 15L12 21L18 15" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    <circle cx="6" cy="9" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="18" cy="9" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="6" cy="15" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="18" cy="15" r="1.5" fill="currentColor" opacity="0.6"/>
  </svg>
)

export const VolatilityIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M3 12C3 12 6 6 12 6C18 6 21 12 21 12" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
    <path d="M3 12C3 12 6 18 12 18C18 18 21 12 21 12" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
    <circle cx="12" cy="12" r="2.5" fill="currentColor" opacity="0.8"/>
    <circle cx="6" cy="6" r="1.5" fill="currentColor" opacity="0.5"/>
    <circle cx="18" cy="6" r="1.5" fill="currentColor" opacity="0.5"/>
    <circle cx="6" cy="18" r="1.5" fill="currentColor" opacity="0.5"/>
    <circle cx="18" cy="18" r="1.5" fill="currentColor" opacity="0.5"/>
  </svg>
)

export const TrendFollowingIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M3 20L9 14L13 18L21 10" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M21 6V10H17" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    <circle cx="9" cy="14" r="2" fill="currentColor" opacity="0.6"/>
    <circle cx="13" cy="18" r="2" fill="currentColor" opacity="0.6"/>
    <circle cx="21" cy="10" r="2" fill="currentColor" opacity="0.6"/>
  </svg>
)

export const GridTradingIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <rect x="3" y="3" width="18" height="18" stroke="currentColor" strokeWidth="2.5" rx="1"/>
    <path d="M3 9H21" stroke="currentColor" strokeWidth="2.5"/>
    <path d="M3 15H21" stroke="currentColor" strokeWidth="2.5"/>
    <path d="M9 3V21" stroke="currentColor" strokeWidth="2.5"/>
    <path d="M15 3V21" stroke="currentColor" strokeWidth="2.5"/>
    <circle cx="6" cy="6" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="12" cy="6" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="18" cy="6" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="6" cy="12" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="12" cy="12" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="18" cy="12" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="6" cy="18" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="12" cy="18" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="18" cy="18" r="1.5" fill="currentColor" opacity="0.6"/>
  </svg>
)

export const MarketMakingIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M12 2L15 9L22 10L17 15L18 22L12 18L6 22L7 15L2 10L9 9L12 2Z" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="currentColor" fillOpacity="0.1"/>
    <circle cx="12" cy="12" r="2" fill="currentColor" opacity="0.8"/>
    <circle cx="9" cy="9" r="1.5" fill="currentColor" opacity="0.5"/>
    <circle cx="15" cy="9" r="1.5" fill="currentColor" opacity="0.5"/>
  </svg>
)

export const ScalpingIcon = () => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M3 12H21" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"/>
    <path d="M6 8L9 12L6 16" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    <path d="M18 8L15 12L18 16" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
    <circle cx="12" cy="12" r="2" fill="currentColor" opacity="0.8"/>
    <circle cx="6" cy="8" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="18" cy="8" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="6" cy="16" r="1.5" fill="currentColor" opacity="0.6"/>
    <circle cx="18" cy="16" r="1.5" fill="currentColor" opacity="0.6"/>
  </svg>
)

